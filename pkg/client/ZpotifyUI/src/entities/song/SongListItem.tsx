    ...
    return (
        ...
        >
            <div className={cls.NumColumn}>
                {isPlaying && active ? <NowPlayingBars /> : <span className={cls.TrackNum}>{num}</span>}
            </div>
            ...
            <div className={cls.Titles}>
                ...
                <div className={cls.Artists}>{song.artists?.map((a) => a.name).join(', ')}</div>
            </div>

            <div className={cls.Duration}>{formatDuration(song.durationSec ?? 0)}</div>

            <div className={cls.MoreDotsWrapper}>
                <MoreButton ops={menuOps} onOpen={handleMenuOpen} onClose={handleMenuClose} />
            </div>
        </div>
    );
}
