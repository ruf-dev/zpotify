...
type TelegramIdentityStorage interface {
	...
	GetByUserId(ctx context.Context, userId int64) (domain.TelegramIdentity, error)
}
...
type SongStorage interface {
	...
	SearchByTitle(ctx context.Context, query string, userId int64, limit, offset uint64) ([]domain.Song, error)
