...
func TestPlaylistService_Search_SplitsAlbumsFromPlaylists(t *testing.T) {
	album := domain.PlaylistSearchResult{
		Playlist: domain.Playlist{
			Uuid:    "album-uuid",
			Name:    "Some Album",
			Artists: []domain.ArtistsBase{{Uuid: "artist-uuid", Name: "Some Artist"}},
		},
		Score: 0.9,
	}
	...
	albums, playlists, err := svc.Search(context.Background(), "some", 10, 0)
	...
	assert.Equal(t, "album-uuid", albums[0].Uuid)

	require.Len(t, playlists, 1)
	assert.Equal(t, "playlist-uuid", playlists[0].Uuid)
}
...
func TestPlaylistService_Search_PropagatesStorageError(t *testing.T) {
	...
	_, _, err := svc.Search(context.Background(), "some", 10, 0)
	require.Error(t, err)
}
