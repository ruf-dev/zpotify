...
type Impl struct {
	zpotify_api.UnimplementedArtistsAPIServer
...
func New(srv service.Service) *Impl {
	return &Impl{
		UnimplementedArtistsAPIServer: zpotify_api.UnimplementedArtistsAPIServer{},
		artistsService:                srv.ArtistsService(),
	}
}

func (impl *Impl) Register(server grpc.ServiceRegistrar) {
	zpotify_api.RegisterArtistsAPIServer(server, impl)
}

func (impl *Impl) Gateway(ctx context.Context, endpoint string, opts ...grpc.DialOption) (route string, handler http.Handler) {
	gwHttpMux := runtime.NewServeMux()

	err := zpotify_api.RegisterArtistsAPIHandlerFromEndpoint(
		ctx,
		gwHttpMux,
		endpoint,
		opts,
